...
pub fn build_pdf(
    ...
    for i in 0..page_count {
        ...
        let body = format!(
            ...
            cid = cid,
            annots = {
                // Merge hyperlink annotations (from `annots`) and form-field
                // widgets (`page_annots`) into a single /Annots array.
                let mut s = annots;
                let fa = page_annots[i as usize]
                    .iter()
                    .map(|id| format!("{} 0 R", id))
                    .collect::<Vec<_>>();
                if !fa.is_empty() {
                    if s.is_empty() {
                        s = format!("/Annots [{}]", fa.join(" "));
                    } else {
                        let inner = s.trim().trim_start_matches("/Annots [").trim_end_matches(']');
                        s = format!("/Annots [{} {}]", inner, fa.join(" "));
                    }
                }
                if s.is_empty() {
                    String::new()
